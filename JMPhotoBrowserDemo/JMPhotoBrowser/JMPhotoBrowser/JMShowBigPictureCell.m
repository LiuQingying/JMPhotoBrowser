...
@interface JMShowBigPictureCell()
...
- (instancetype)initWithFrame:(CGRect)frame{
    if(self = [super initWithFrame:frame]){
        _bigPictureView = [[JMBigPictureView alloc] initWithFrame:self.bounds];
        [self addSubview:_bigPictureView];
...
- (void)setImageWithURL:(NSString *)url placeholderImage:(UIImage *)placeholder{
    ...
    self.currenPictureUrl = newURL.absoluteString;
    NSLog(@"----+++%@",[NSThread currentThread]);
    
    NSData *data = [self imageDataFromDiskCacheWithKey:newURL.absoluteString];
